...
class VAT(models.MultiModel):
    ...
    def model(self, lr, wd, ema, warmup_pos, vat, vat_eps, batch, entmin_weight, method, **kwargs):
        ...
        if method ==1:
            loss_entmin = -tf.reduce_sum(tf.svd(tf.nn.softmax(logits_y),compute_uv=False))/batch
        else:
        ...
        train_op = tf.train.AdamOptimizer(lr).minimize(loss + entmin_weight * loss_entmin,
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float('method', 1, 'VAT perturbation size.')
    ...
    FLAGS.set_default('train_kimg', 1 << 15)
    app.run(main)
