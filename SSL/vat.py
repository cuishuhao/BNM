...
class VAT(models.MultiModel):
    ...
    def model(self, lr, wd, ema, warmup_pos, vat, vat_eps, entmin_weight, method, **kwargs):
        hwc = [self.dataset.height, self.dataset.width, self.dataset.colors]
        x_in = tf.placeholder(tf.float32, [None] + hwc, 'x')#.gpu()
        y_in = tf.placeholder(tf.float32, [None] + hwc, 'y')#.gpu()
        l_in = tf.placeholder(tf.int32, [None], 'labels')#.gpu()
        wd *= lr
        ...
        if method==1:
            loss_entmin = -tf.reduce_sum(tf.svd(tf.nn.softmax(logits_y),compute_uv=False))/100
        else:
            loss_entmin = tf.reduce_mean(tf.distributions.Categorical(logits=logits_y).entropy())
        #loss_entmin = -tf.reduce_mean(tf.svd(logits_y,compute_uv=False))
        #loss_entmin = -tf.reduce_mean(tf.log(1-logits_y)+tf.log(logits_y))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float('method', 1, 'Entropy minimization weight.')
